...
class StageManager:
    ...
    # -----------------------------
    # UI stage descriptions (fixed garbled JP text)
    # -----------------------------
    @staticmethod
...
class TUTORIAL_PG_Properties(PropertyGroup):
    ...
    log_dir: StringProperty(
        name="ログ保存フォルダ",
        description="クリックでフォ���ダ選択（環境によってFile Browserが落ちる場合あり：下の安全ボタンを使用）",
        subtype='DIR_PATH',
...
class TUTORIAL_OT_export_stage_summary_csv(Operator):
    ...
    def execute(self, context):
        ...
        if not props.participant_log_path:
            self.report({'ERROR'}, "ログファイルがありません。参加者IDとログ保存フォルダを設定し、確認を1回以上実行してください。")
            return {'CANCELLED'}
    ...
    def execute(self, context):
        ...
        try:
            if ch == 1:
                ...
                # Stage 1 only; reset saved path + create camera + create sun
                props.current_stage = 1
...
class TUTORIAL_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.label(text=f"停滞時間: {StageManager.get_stall_seconds(context):.1f}s / 失敗回数: {props.failed_validate_count}")
