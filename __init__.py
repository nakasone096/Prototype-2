...
class TUTORIAL_OT_export_stage_summary_csv(Operator):
    ...
    def execute(self, context):
        ...
        # NEW: CSV出力時にログファイルを自動作成（確認を押さなくてもOK）
...
class TUTORIAL_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        # Idle time display (always visible)
        props.current_stall_seconds = StageManager.get_stall_seconds(context)
        layout.separator()
        layout.label(text=f"停滞時間: {props.current_stall_seconds:.1f}s / 失敗回数: {props.failed_validate_count}")
...
def register():
    ...
    print("✓ 3DCG Tutorial Simulator registered")
